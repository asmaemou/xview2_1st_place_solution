...
import cv2  # ✅ for robust triplet validation
...
from idabd_stage2.aug.unsharp_edge import fuse_unsharp_edge_post


CSV_FIELDS = [
    "seed","init_weight","loc_weight","loc_platt","loc_thresh",
    "amp",
    "fusion_p","alpha_unsharp","unsharp_sigma","unsharp_amount","alpha_edge","canny_t1","canny_t2","edge_dilate",
...
class PostAugUnsharpEdge:
    """
    Picklable callable for Windows DataLoader workers (if workers>0),
    and safe for workers=0 too.
    """
    def __init__(self, alpha_unsharp, unsharp_sigma, unsharp_amount,
    ...
    def __call__(self, post_bgr):
        out = fuse_unsharp_edge_post(
            post_bgr,
...
def filter_valid_triplets(triplets, max_report=10):
    """
    Remove any triplet where paths are missing OR post image can't be read by cv2.
    This prevents: post_used=None -> pad_to_factor crash.
    """
    kept = []
    bad = []
...
def main():
    ...
    # TRAIN-only Unsharp+Edge
    ap.add_argument("--fusion_p", type=float, default=0.75)
    ap.add_argument("--alpha_unsharp", type=float, default=0.45)
    ...
    ap.add_argument("--out_dir", default="idabd_stage2_damage_ft_checkpoints_RGB_UNSHARP_EDGE_ONLY")
    ap.add_argument("--loc_weight", default="")
    ap.add_argument("--loc_platt", default="")
    ap.add_argument("--loc_thresh", type=float, default=0.5)

    ap.add_argument("--csv_path", default="idabd_stage2_pipeline_results_RGB_UNSHARP_EDGE_ONLY.csv")
    ...
    if args.init_weight:
        ...
        init_weights, _ = auto_find_stage2_init_weights(
    ...
    # ✅ prevent post_used=None crashes by removing unreadable triplets
    triplets = filter_valid_triplets(triplets)
    if not triplets:
        raise FileNotFoundError("All triplets are invalid (missing/unreadable post images). Check dataset paths.")
    ...
    # ✅ instantiate augmentation ONCE (NOT inside a def)
    post_aug = PostAugUnsharpEdge(
    ...
    # datasets: train uses aug; val/test raw
    train_ds = IdaBDStage2Dataset6CH(
        ...
        post_transform_train=post_aug,   # ✅ correct callable
    ...
    for init_w in init_weights:
        ...
        row = {
            ...
            "unsharp_amount": args.unsharp_amount,
            "alpha_edge": args.alpha_edge,
            "canny_t1": args.canny_t1, "canny_t2": args.canny_t2, "edge_dilate": args.edge_dilate,
...
if __name__ == "__main__":
    mp.freeze_support()
    # On Windows, spawn is default; forcing is fine but keep it only here.
