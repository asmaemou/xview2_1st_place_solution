...
from idabd_stage2.aug.contrast_only import histeq_pair
...
def main():
    ...
    # Contrast-only (HistEq) probability
    ...
    ap.add_argument("--out_dir", default="idabd_stage2_damage_ft_checkpoints_RGB_CONTRAST_ONLY")
    ap.add_argument("--loc_weight", default="")
    ap.add_argument("--loc_platt", default="")
    ap.add_argument("--loc_thresh", type=float, default=0.5)

    ap.add_argument("--csv_path", default="idabd_stage2_pipeline_results_RGB_CONTRAST_ONLY.csv")
    ...
    # Pair transform (HistEq for BOTH pre & post)
    def pair_tf(pre_bgr, post_bgr):
        return histeq_pair(pre_bgr, post_bgr)

    # Match your current script: TRAIN-only augmentation
    eval_tf = None
