...
from idabd_stage2.aug.edges_only import fuse_rgb_edges_pair
...
def ensure_min_destroyed(val_tr, train_tr, min_val=1, min_train=1):
    ...
    # Ensure TRAIN has at least min_train destroyed tiles (swap from val if needed)
    if min_train > 0:
        train_d = [t for t in train_tr if has_destroyed(t[2])]
...
def main():
    ...
    ap.add_argument("--out_dir", default="idabd_stage2_damage_ft_checkpoints_RGB_EDGES_ONLY")
    ...
    ap.add_argument("--csv_path", default="idabd_stage2_pipeline_results_RGB_EDGES_ONLY.csv")
