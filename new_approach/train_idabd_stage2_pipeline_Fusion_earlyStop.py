...
UPDATED BEHAVIOR:
- Running with NO ARGS works:
    python train_idabd_stage2_pipeline_Fusion_earlyStop.py
  -> defaults to --train_all and runs damage-only mode (no stage-1 loc gating)

- Stage-1 localization is OPTIONAL:
  If --loc_weight is provided: pipeline gating uses predicted building mask.
  If --loc_weight is NOT provided: evaluation uses GT building mask (oracle gating)
  and localization metrics are NaN.

FUSION AUGMENTATION (paper-style):
- Derived from grayscale:
    1) Gradient edge band (Sobel magnitude)
    2) Contrast enhancement band (CLAHE)
    3) Unsharp mask band
- Fused back into RGB (3ch), so architecture stays unchanged.
================================================================================
"""
...
# =============================================================================
# Fusion augmentation (paper-style: grayscale -> edge/contrast/unsharp)
# =============================================================================
def _sobel_edge_band(gray: np.ndarray) -> np.ndarray:
...
def _clahe_band(gray: np.ndarray, clip=2.0, tile=(8, 8)) -> np.ndarray:
    clahe = cv2.createCLAHE(clipLimit=float(clip), tileGridSize=tuple(tile))
    return clahe.apply(gray)


def _unsharp_band(gray: np.ndarray, amount=1.0, radius=3) -> np.ndarray:
    blur = cv2.GaussianBlur(gray, (0, 0), float(radius))
    sharp = cv2.addWeighted(gray, 1.0 + float(amount), blur, -float(amount), 0)
    return np.clip(sharp, 0, 255).astype(np.uint8)


def fusion_augment_paper(img_bgr: np.ndarray) -> np.ndarray:
    """
    Keeps 3 channels (RGB/BGR) for compatibility with existing 6ch pre/post models.
    Creates grayscale-derived bands (edge/contrast/unsharp) and fuses into RGB.
    """
    gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)

    edge = _sobel_edge_band(gray)
    con  = _clahe_band(gray, clip=2.0, tile=(8, 8))
    ush  = _unsharp_band(gray, amount=1.0, radius=3)

    # Convert bands to 3ch so we can fuse with BGR
    edge3 = cv2.cvtColor(edge, cv2.COLOR_GRAY2BGR)
    con3  = cv2.cvtColor(con,  cv2.COLOR_GRAY2BGR)
    ush3  = cv2.cvtColor(ush,  cv2.COLOR_GRAY2BGR)

    # Paper describes stacking channels; here we fuse (weighted) to keep model unchanged.
    # Weights chosen to preserve natural appearance while emphasizing structure.
    img_f = img_bgr.astype(np.float32)
    fused = (
        0.60 * img_f +
        0.15 * edge3.astype(np.float32) +
        0.15 * con3.astype(np.float32) +
        0.10 * ush3.astype(np.float32)
    )
    return np.clip(fused, 0, 255).astype(np.uint8)
...
class IdaBDStage2Damage(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train and (self.rng.random() < self.fusion_p):
            # Apply to both pre and post (same as your wording)
            pre  = fusion_augment_paper(pre)
            post = fusion_augment_paper(post)
...
# =============================================================================
# PIPELINE evaluation (Stage-1 loc -> Stage-2 damage) OR damage-only mode
# =============================================================================
@torch.no_grad()
def eval_loader_pipeline(model, loader, device, loc_model=None, loc_a=1.0, loc_b=0.0, loc_thresh=0.5, calib=None, amp=False):
    model.eval()
    ...
    for x, raw, _ in loader:
        ...
        # Optional stage-1 gating
        if loc_model is None:
            # Damage-only evaluation: oracle building mask from GT
        ...
        # Localization metrics only if loc_model exists
        gt_build = valid & torch.isin(raw, build_tensor)
        pr_build = valid & build_mask
...
def train_and_eval_pipeline_one(args, init_weight: str):
    ...
    # -------------------------
    # OPTIONAL: Load Stage-1 localization
    # -------------------------
    loc_model = None
    loc_a, loc_b = 1.0, 0.0
    ...
    if args.loc_weight:
        ...
        print("[LOC  ] DISABLED (no --loc_weight). Damage-only mode.")
        print("[LOC  ] Eval uses GT building mask; localization metrics will be NaN.")
    ...
    def print_pipeline(title, acc5, macroB, f1sD, locP, locR, locF):
        print(f"\n==================== {title} ====================")
        if np.isnan(locF):
            print("END-TO-END: Damage-only evaluation (GT building mask gating).")
            print("Localization metrics: NaN (Stage-1 loc disabled).")
        else:
            print("END-TO-END: Stage-1 loc gating -> Stage-2 damage (VALID pixels only; mask!=255)")
            print(f"F1 Localization (Building vs Background)={locF:.6f}  (P={locP:.6f}, R={locR:.6f})")
    ...
    print_pipeline("TEST (UNCALIBRATED DAMAGE)", acc_u, macroB_u, f1sD_u, locP_u, locR_u, locF_u)
    print_pipeline("TEST (CALIBRATED DAMAGE)",   acc_c, macroB_c, f1sD_c, locP_c, locR_c, locF_c)
...
def main():
    ...
    # AUTO-RUN: if user ran with no args, do something sensible
    if (not args.train_all) and (not args.init_weight):
        print("[INFO] No --init_weight and --train_all not set. Defaulting to --train_all.")
        args.train_all = True
