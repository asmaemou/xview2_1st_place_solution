...
"""
================================================================================
STAGE-2 DAMAGE (IDABD) — FUSION-ONLY (NO RAW RGB) + NO DA
Fusion-only input:
  x6 = [pre_fusion_3ch, post_fusion_3ch]   (still 6-channel for *_Unet_Double)
Where fusion_3ch is built from:
  - Edge (Canny + dilation) as overlay
  - Contrast (CLAHE)
  - Unsharp mask
NO DA:
  - NO calibration / vector scaling
  - Evaluate uncalibrated only
...
RUN:
  python train_idabd_stage2_pipeline_FUSION_ONLY_edge_contrast_unsharp_noDA_earlyStop.py
================================================================================
"""
...
def preprocess_rgb(img_bgr: np.ndarray) -> torch.Tensor:
    # NOTE: even though we call this rgb, the input will be "fusion-only" 3ch image
...
# =============================================================================
# Fusion (build a 3-channel "fusion-only" image)
# =============================================================================
...
def fusion_only_bgr(
    ...
    """
    Returns a 3-channel "fusion-only" image. It is NOT raw RGB, but it is 3-channel.
    """
...
def preprocess_6ch_fusion_only(pre_bgr: np.ndarray, post_bgr: np.ndarray, fusion_cfg: dict) -> torch.Tensor:
    ...
    return torch.cat([pre_t, post_t], dim=0)  # 6xHxW


# =============================================================================
# Dataset: destroyed-aware crops + fusion-only pre/post
# =============================================================================
class IdaBDStage2DamageFusionOnly(Dataset):
    def __init__(
        self,
        triplets,
        crop_size=512,
    ...
    def _random_crop(self, pre, post, mask, size):
        ...
        for _ in range(self.crop_attempts):
            ...
            if want_destroyed:
                if int((m == 4).sum()) >= self.min_destroyed_px:
                    return pre[y0:y0+size, x0:x0+size], post[y0:y0+size, x0:x0+size], m
            else:
                if int(np.isin(m, [1, 2, 3, 4]).sum()) >= self.min_build_px:
                    return pre[y0:y0+size, x0:x0+size], post[y0:y0+size, x0:x0+size], m
    ...
    def __getitem__(self, idx):
        ...
        # Apply fusion-only stochastically in train, always in val/test
        do_fuse = (self.rng.random() < self.fusion_p) if self.is_train else True

        if do_fuse:
            x = preprocess_6ch_fusion_only(pre, post, self.fusion_cfg)
        else:
            # If not fusing (rare), fall back to fusing anyway because "fusion-only" is the experiment definition
            x = preprocess_6ch_fusion_only(pre, post, self.fusion_cfg)

        pre_pad, pad_hw = pad_to_factor(pre, 32)
        post_pad, _ = pad_to_factor(post, 32)
        if pad_hw != (0, 0):
...
# =============================================================================
# Checkpoint loading + model construction
# =============================================================================
...
# =============================================================================
# Pipeline evaluation (uncal only)
# =============================================================================
...
@torch.no_grad()
def eval_loader_pipeline_uncal(model, loader, device, loc_model, loc_a, loc_b, loc_thresh, amp=False):
    model.eval()
    loc_model.eval()
    ...
    return acc5, macro_damage, f1s_damage, loc_p, loc_r, loc_f1
...
def train_and_eval_pipeline_one(args, init_weight: str):
    ...
    train_ds = IdaBDStage2DamageFusionOnly(
        ...
        fusion_p=args.fusion_p, fusion_cfg=fusion_cfg,
    )
    val_ds = IdaBDStage2DamageFusionOnly(
        val_tr, crop_size=0, is_train=False, min_build_px=0, seed=args.seed,
        fusion_p=1.0, fusion_cfg=fusion_cfg,
    )
    test_ds = IdaBDStage2DamageFusionOnly(
    ...
    # Load best and evaluate pipeline (uncal only)
...
def main():
    ...
    ap.add_argument("--out_dir", default="idabd_stage2_damage_ft_checkpoints_FUSION_ONLY_edge_contrast_unsharp_NO_DA")
    ...
    ap.add_argument("--csv_path", default="idabd_stage2_pipeline_results_FUSION_ONLY_edge_contrast_unsharp_NO_DA.csv")
