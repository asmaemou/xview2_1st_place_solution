...
"""
================================================================================
IDABD STAGE-2 DAMAGE (6-CH: preRGB + postRGB_used) + SUPERVISED DOMAIN ADAPTATION
+ DESTROYED-AWARE CROPS + (OPTIONAL) CLASS WEIGHTS
+ VAL LOGIT CALIBRATION (VECTOR SCALING, CLASS-BALANCED SAMPLING)
+ PIPELINE-ONLY EVALUATION USING STAGE-1 LOCALIZATION (WITH OPTIONAL PLATT)

✅ Zero-args runnable:
    python train_idabd_stage2_pipeline_RGBplusFUSION.py

Why 6 channels (NOT 12)?
- The xView2 1st place Stage-2 models (Unet_Double in zoo/models.py) are hard-coded
  to split the input into two 3-channel branches: x[:,:3] and x[:,3:6].
- Therefore Stage-2 input MUST be 6 channels to avoid channel mismatch.

Stage-2 input (6 channels):
  x6 = [pre_rgb(3), post_rgb_used(3)]  => 6xHxW

Fusion behavior:
  - pre_rgb is always raw.
  - post_rgb_used is:
      * fusion_augment(post) with prob --fusion_p during TRAIN
      * raw post during VAL/TEST (fusion disabled)

SDA (supervised domain adaptation):
  - Load source-pretrained Stage-2 checkpoint (--init_weight)
  - Fine-tune on labeled IDABD (target domain)

Mask assumptions (POST masks):
0=background, 1=No Damage, 2=Minor, 3=Major, 4=Destroyed, 255=ignore
================================================================================
"""
...
STAGE2_IN_CH = 6  # IMPORTANT: Unet_Double expects 6ch and internally splits into 3+3


# ---------------------------------------------------------------------
# CSV fields (pipeline-only)
# ---------------------------------------------------------------------
CSV_FIELDS = [
    "seed",
    "init_weight",
...
# =============================================================================
# Fusion augmentation (applied to post image only in this 6ch pipeline)
# =============================================================================
...
# =============================================================================
# Dataset (Stage-2): DESTROYED-AWARE CROPS + 6CH INPUT
# =============================================================================
class IdaBDStage2DamageRGBPlusFusion6CH(Dataset):
    def __init__(
        self,
        triplets,
        crop_size=512,
    ...
    def __getitem__(self, idx):
        ...
        # Fusion only affects the POST half of the 6ch input
        if self.is_train and (self.rng.random() < self.fusion_p):
            post_used = fusion_augment(post)
        else:
            post_used = post

        x = preprocess_6ch(pre, post_used)
...
# =============================================================================
# Build models from weights (NO input-channel surgery; Unet_Double expects 6ch)
# =============================================================================
...
def train_and_eval_pipeline_one(args, init_weight: str):
    ...
    print("[SDA  ] Fine-tuning source-pretrained weights on labeled IDABD.")
    ...
    loc_a, loc_b = 1.0, 0.0
    if args.loc_platt and path.exists(args.loc_platt) and args.loc_platt.lower().endswith(".npz"):
        try:
            ...
            if ("a" in d) and ("b" in d):
                ...
                print(f"[WARN] Platt missing a/b: {args.loc_platt} -> using a=1,b=0")
        except Exception as e:
            print(f"[WARN] Could not load platt ({args.loc_platt}): {e} -> using a=1,b=0")
    else:
        if args.loc_platt:
            print(f"[WARN] loc_platt not valid .npz: {args.loc_platt} -> using a=1,b=0")
        else:
    ...
    train_ds = IdaBDStage2DamageRGBPlusFusion6CH(
        train_tr, crop_size=args.crop, is_train=True, fusion_p=args.fusion_p,
        min_build_px=args.min_build_px, seed=args.seed,
        focus_destroyed_p=args.focus_destroyed_p, min_destroyed_px=args.min_destroyed_px,
        crop_attempts=args.crop_attempts,
    )
    val_ds   = IdaBDStage2DamageRGBPlusFusion6CH(val_tr, crop_size=0, is_train=False, fusion_p=0.0, min_build_px=0, seed=args.seed)
    test_ds  = IdaBDStage2DamageRGBPlusFusion6CH(test_tr, crop_size=0, is_train=False, fusion_p=0.0, min_build_px=0, seed=args.seed)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save({
            ...
            "val_loss": val_loss,
            "stage2_in_channels": STAGE2_IN_CH,
        }, last_ckpt_path)
    ...
    print(f"[CALIB] Fitting vector scaling on N={X.shape[0]} pixels...")
    W_np, b_np = fit_vector_scaling(X, Y, device=device, lr=args.calib_lr,
                                    epochs=args.calib_epochs, batch_size=args.calib_batch, wd=args.calib_wd)
    ...
    print_pipeline("PIPELINE TEST (UNCALIBRATED DAMAGE + STAGE-1 LOC) [6CH preRGB + postRGB_used]",
                   acc_u, macroB_u, f1sD_u, locP_u, locR_u, locF_u)
    print_pipeline("PIPELINE TEST (CALIBRATED DAMAGE + STAGE-1 LOC) [6CH preRGB + postRGB_used]",
...
# =============================================================================
def _pick_most_recent(paths_):
    return max(paths_, key=lambda p: os.path.getmtime(p))


def auto_find_loc_weight(default_dir=STAGE1_DIR_DEFAULT):
    patterns = [
        path.join(default_dir, "**", "*_idabd_ft_best.pth"),
        path.join(default_dir, "**", "*_idabd_ft_last.pth"),
        path.join(default_dir, "**", "*loc*_best*.pth"),
        path.join(default_dir, "**", "*loc*_last*.pth"),
    ]
    cands = []
    for ptn in patterns:
        cands += glob.glob(ptn, recursive=True)
    cands = [c for c in cands if path.isfile(c)]
    if not cands:
        return ""
    return _pick_most_recent(cands)


def auto_find_loc_platt(loc_weight_path, default_dir=STAGE1_DIR_DEFAULT):
    if loc_weight_path and loc_weight_path.lower().endswith(".pth"):
        base = path.basename(loc_weight_path)
        guesses = [
            base.replace("_idabd_ft_best.pth", "_idabd_platt.npz"),
            base.replace("_idabd_ft_last.pth", "_idabd_platt.npz"),
            base.replace(".pth", "_idabd_platt.npz"),
        ]
        for g in guesses:
            gp = path.join(path.dirname(loc_weight_path), g)
            if path.exists(gp) and gp.lower().endswith(".npz"):
                return gp

    cands = glob.glob(path.join(default_dir, "**", "*_idabd_platt.npz"), recursive=True)
    cands = [c for c in cands if path.isfile(c)]
    if not cands:
        return ""
    return _pick_most_recent(cands)
...
def auto_find_stage2_init_weight(weights_dir):
    patterns = [
        "dpn92_cls*.pth",
        "res34_cls*.pth",
        "res34_cls2*.pth",
    ...
    cands = []
    for ptn in patterns:
        cands += glob.glob(path.join(weights_dir, ptn))
    cands = [c for c in cands if path.isfile(c)]
    if not cands:
        return ""
    return _pick_most_recent(sorted(cands))
...
def main():
    ...
    if not path.isdir(args.img_dir):
        raise FileNotFoundError(f"--img_dir not found: {args.img_dir}")
    if not path.isdir(args.mask_dir):
        raise FileNotFoundError(f"--mask_dir not found: {args.mask_dir}")

    if not args.loc_weight:
        args.loc_weight = auto_find_loc_weight(args.stage1_dir)
        if args.loc_weight:
            print(f"[AUTO ] --loc_weight -> {args.loc_weight}")

    if not args.loc_weight or not path.exists(args.loc_weight):
        raise FileNotFoundError("Stage-1 localization weight not found. Pass --loc_weight or place it in stage1_dir.")

    if not args.loc_platt:
        args.loc_platt = auto_find_loc_platt(args.loc_weight, args.stage1_dir)
        if args.loc_platt:
            print(f"[AUTO ] --loc_platt  -> {args.loc_platt}")
    ...
    print(f"[STAGE2] Input channels = {STAGE2_IN_CH} (preRGB + postRGB_used)")
    ...
    if args.train_all:
        ...
        if not args.init_weight:
            args.init_weight = auto_find_stage2_init_weight(args.weights_dir)
            if args.init_weight:
                print(f"[AUTO ] --init_weight -> {args.init_weight}")

        if not args.init_weight or not path.exists(args.init_weight):
            raise FileNotFoundError("Stage-2 init weight not found. Pass --init_weight or put *_cls*.pth in ./weights")
