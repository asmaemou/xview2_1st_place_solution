...
# """
# THIS NEW VERSION INCLUDE THE FIRST PLACE SOLUTION MODEL
#!/usr/bin/env python3
"""
================================================================================
EVAL: ENSEMBLE STAGE-1 LOCALIZATION + ENSEMBLE STAGE-2 DAMAGE
IDABD end-to-end pipeline

Supports two Stage-2 modes:

1) Calibrated Ida-BD mode:
   - Stage-2 checkpoints require vector-scaling calibration .npz files.
   - Used by the RGB + Unsharp Model.

2) Uncalibrated released-weight mode:
   - Stage-2 checkpoints are used directly without calibration .npz files.
   - Used by the xView2 1st Place Released Weights.

Stage-1 ensemble:
  - Load multiple localization models.
  - For each model: p(building) = sigmoid(a*logit + b)
  - If Platt scaling exists, use it.
  - Otherwise use a=1, b=0.
  - Ensemble by averaging probabilities.
  - build_mask = mean_prob >= loc_thresh

Stage-2 ensemble:
  - Load multiple damage models.
  - If calibration is required, apply Vector Scaling calibration.
  - If calibration is disabled, use raw logits directly.
  - Convert logits to softmax probabilities over damage classes 1..4.
  - Ensemble by averaging probabilities.
  - damage_pred = argmax(mean_probs) + 1
...
def predict_damage_ensemble_cal(stage2_pack, x6, use_amp: bool):
    """
    The name is kept for compatibility with your existing inference script.

    stage2_pack entries are:
      (model, W, b)

    If W and b are tensors:
      apply calibrated vector scaling.

    If W and b are None:
      use raw logits directly.
    """
...
# =============================================================================
@torch.no_grad()
def eval_pipeline_ensemble_cal(
    loader,
    device,
    loc_pack,
    stage2_pack,
    ...
    for x6, raw, _ in loader:
        ...
        pred_damage = predict_damage_ensemble_cal(
            stage2_pack,
            x6,
            use_amp=use_amp,
        )
    ...
    if debug_destroyed:
        ...
        print(
            f"[DESTROYED DEBUG] GT={gt}  PRED={pred}  "
            f"TP={tp}  FP={fp}  FN={fn}"
        )
...
def load_loc_pack(loc_dir: str, device, loc_glob="*_idabd_ft_best.pth"):
    ...
    if not weights:
        raise FileNotFoundError(
            f"No Stage-1 loc weights found in {loc_dir} with {loc_glob}"
        )
    ...
    for w in weights:
        ...
        if base.endswith("_idabd_ft_best.pth"):
            stem = base[:-len("_idabd_ft_best.pth")]
        else:
...
def stage2_calibration_path(stage2_dir: str, checkpoint_path: str) -> str:
    """
    Build the expected Ida-BD calibration filename.

    Example:
      dpn92_cls_cce_0_tuned_best_idabd_stage2_ft_best.pth
    becomes:
      dpn92_cls_cce_0_tuned_best_idabd_stage2_calib_vector_scaling.npz
    """

    base = path.basename(checkpoint_path)

    if base.endswith("_idabd_stage2_ft_best.pth"):
        stem = base[:-len("_idabd_stage2_ft_best.pth")]
    else:
        stem = path.splitext(base)[0]

    return path.join(stage2_dir, f"{stem}_idabd_stage2_calib_vector_scaling.npz")


def load_stage2_pack(
    stage2_dir: str,
    device,
    stage2_glob="*_idabd_stage2_ft_best.pth",
    stage2_max_models: int = 12,
    ...
    if not ckpts:
        raise FileNotFoundError(
            f"No Stage-2 best checkpoints found in {stage2_dir} ({stage2_glob})"
        )

    if stage2_max_models > 0 and len(ckpts) > stage2_max_models:
        print(
            f"[STAGE2] WARNING: found {len(ckpts)} models; "
            f"keeping first {stage2_max_models} (sorted)."
        )
        ckpts = ckpts[:stage2_max_models]
    ...
    for ck in ckpts:
        calib = stage2_calibration_path(stage2_dir, ck)
        ...
        if path.exists(calib):
            d = np.load(calib)

            W = torch.from_numpy(d["W"].astype(np.float32)).to(device)
            b = torch.from_numpy(d["b"].astype(np.float32)).to(device)

            print(f"[STAGE2] Calibration loaded for {path.basename(ck)}")
        else:
            if require_calibration:
                raise FileNotFoundError(
                    f"Missing calibration for {ck} -> expected {calib}"
                )
            ...
            print(
                f"[STAGE2] Calibration disabled or unavailable for "
                f"{path.basename(ck)}. Using raw logits."
            )
...
def main():
    ...
    ap.add_argument(
        "--loc_dir",
        required=True,
        help="Folder containing localization weights.",
    )

    ap.add_argument(
        "--loc_glob",
        default="*_idabd_ft_best.pth",
        help="Pattern to select Stage-1 localization weights.",
    )

    ap.add_argument(
        "--stage2_dir",
        required=True,
        help="Folder containing Stage-2 damage weights.",
    )

    ap.add_argument(
        "--stage2_glob",
        default="*_idabd_stage2_ft_best.pth",
        help="Pattern to select Stage-2 damage weights.",
    )
    ...
    ap.add_argument(
        ...
        type=int,
        default=12,
        help="If >0, cap how many Stage-2 models are loaded.",
    )
    ...
    ap.add_argument(
        ...
        default="required",
        help=(
            "required = calibration .npz files must exist. "
            "none = use raw logits without calibration."
        ),
    )
    ...
    ap.add_argument(
        "--csv_path",
        default="idabd_pipeline_ENSEMBLE_LOC+DMG_CAL_ONLY.csv",
    )
    ...
    require_calibration = args.stage2_calibration_mode == "required"

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    print(f"[DEVICE] {device}")
    print(f"[STAGE2_CALIBRATION_MODE] {args.stage2_calibration_mode}")
    ...
    triplets = build_stage2_triplets(
        args.img_dir,
        args.mask_dir,
    ...
    if not triplets:
        raise FileNotFoundError(
            "No (pre, post, mask) triplets found. Check paths."
        )
    ...
    print(
        f"[DATA  ] total={n} | train={len(train_tr)} | "
        f"val={len(val_tr)} | test={len(test_tr)}"
    )
    ...
    print("\n==================== ENSEMBLED PIPELINE ====================")
    print("END-TO-END: Stage-1 ensemble gate -> Stage-2 ensemble damage")
    print(f"Stage-2 calibration mode: {args.stage2_calibration_mode}")
    print(f"pipeline_acc(0..4)={acc5:.6f}")
    print(
        f"F1 Localization (Building vs Background)={locF:.6f} "
        f"(P={locP:.6f}, R={locR:.6f})"
    )
    print(f"macroF1(Damage 1..4, end-to-end conf)={macroD:.6f}")
