...
"""
================================================================================
STAGE-2 DAMAGE CLASSIFICATION (IDABD) — RGB + CONTRAST-ONLY (HISTEQ) AUGMENTATION
...
RUNS WITH JUST:
  python train_idabd_stage2_pipeline_RGB_plus_CONTRAST_only_earlyStop.py

What this experiment is:
- Stage-2 input is standard 6-channel concat: [pre_RGB, post_RGB]
- Training-only augmentation: Histogram Equalization (global contrast) applied per-image
  with probability fusion_p.
- No edges, no unsharp, no CLAHE.
...
# =============================================================================
# CONTRAST-only augmentation: Histogram Equalization
# =============================================================================
...
# =============================================================================
# Dataset (Stage-2): DESTROYED-AWARE CROPS + CONTRAST-only (training only)
# =============================================================================
class IdaBDStage2Damage(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # CONTRAST-only augmentation (training only)
        if self.is_train and (self.rng.random() < self.fusion_p):
            pre = histeq_bgr(pre)
            post = histeq_bgr(post)
...
def train_and_eval_pipeline_one(args, init_weight: str):
    ...
    # Loaders (contrast-only augmentation in train_ds)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if patience > 0 and epoch >= warmup and no_improve >= patience:
            print(f"[EARLY STOP] No VAL improvement for {patience} epochs (min_delta={min_delta}). "
                  f"Stopping at epoch {epoch}. Best epoch {best_epoch} val_loss={best_val:.6f}.")
            break
...
def main():
    ...
    ap.add_argument("--fusion_p", type=float, default=0.75,
                    help="Probability to apply HistEq contrast augmentation during training.")
    ...
    ap.add_argument("--out_dir", default="idabd_stage2_damage_ft_checkpoints_RGB+CONTRAST_ONLY")
    ...
    ap.add_argument("--csv_path", default="idabd_stage2_pipeline_results_RGB+CONTRAST_ONLY.csv")
    ...
    if not weight_files:
        raise FileNotFoundError(
            f"No Stage-2 init weights found in weights_dir='{args.weights_dir}'. Expected patterns: {patterns}"
        )
