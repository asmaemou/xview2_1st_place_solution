...
"""
================================================================================
STAGE-2 DAMAGE CLASSIFICATION (IDABD) — RGB + EDGES-ONLY (CANNY) AUGMENTATION
...
RUNS WITH JUST:
  python train_idabd_stage2_pipeline_RGB_plus_EDGES_only_earlyStop.py

What this experiment is:
- Input to Stage-2 is still 6-channel: [pre_RGB, post_RGB] (normalized ImageNet)
- BUT (training only): each image may be augmented with "RGB + Edges fusion" using Canny
  - No contrast augmentation, no unsharp masking, no CLAHE.
  - fused = 0.90*img + 0.10*edges_rgb   (tunable weights below)
- Stage-1 localization gating uses PRE RGB only (same as your pipeline design).
...
# =============================================================================
# EDGES-only "fusion augmentation"
# =============================================================================
...
def rgb_plus_edges_only(img_bgr: np.ndarray, edge_w: float = 0.10) -> np.ndarray:
    """
    Returns an RGB-like BGR image fused with edges.
    fused = (1-edge_w)*img + edge_w*edges_rgb
    """
    edge_w = float(np.clip(edge_w, 0.0, 1.0))
    e = canny_edges_rgb(img_bgr).astype(np.float32)
...
# =============================================================================
# Dataset (Stage-2): DESTROYED-AWARE CROPS + EDGES-only fusion (training only)
# =============================================================================
class IdaBDStage2Damage(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # EDGES-only fusion augmentation (training only)
        if self.is_train and (self.rng.random() < self.fusion_p):
            pre = rgb_plus_edges_only(pre, edge_w=self.edge_w)
            post = rgb_plus_edges_only(post, edge_w=self.edge_w)
        ...
        x = preprocess_6ch(pre, post)  # still 6ch pre/post RGB (but edges-fused appearance)
...
def train_and_eval_pipeline_one(args, init_weight: str):
    ...
    val_ds   = IdaBDStage2Damage(val_tr, crop_size=0, is_train=False, fusion_p=0.0, edge_w=args.edge_w, min_build_px=0, seed=args.seed)
    test_ds  = IdaBDStage2Damage(test_tr, crop_size=0, is_train=False, fusion_p=0.0, edge_w=args.edge_w, min_build_px=0, seed=args.seed)
...
def main():
    ...
    # EDGES-only fusion parameters
    ap.add_argument("--fusion_p", type=float, default=0.75,
                    help="Probability to apply RGB+Edges fusion to each image during training.")
    ap.add_argument("--edge_w", type=float, default=0.10,
                    help="Edge mixing weight: fused=(1-edge_w)*RGB + edge_w*EdgesRGB.")
    ...
    # optional single init weight
    ap.add_argument("--init_weight", default="")
    ap.add_argument("--include_idabd_finetune", action="store_true")

    ap.add_argument("--out_dir", default="idabd_stage2_damage_ft_checkpoints_RGB+EDGES_ONLY")
    ...
    ap.add_argument("--csv_path", default="idabd_stage2_pipeline_results_RGB+EDGES_ONLY.csv")
