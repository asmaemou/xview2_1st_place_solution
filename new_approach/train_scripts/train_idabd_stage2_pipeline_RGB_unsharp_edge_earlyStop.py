"""
================================================================================
STAGE-2 DAMAGE CLASSIFICATION (IDABD) — RGB + UNSHARP + EDGE ONLY (NO OTHER FUSION)
...
RUNS WITH JUST:
  python train_idabd_stage2_pipeline_RGB_unsharp_edge_only_earlyStop.py

Default behavior matches what you asked to "bake in":

  AMP: enabled by default
  TRAIN fusion (Unsharp+Edge): fusion_p=0.75
    alpha_unsharp=0.45
    alpha_edge=0.10
    edge_dilate=0
    canny_t1=80
  ...
  Ensure destroyed tiles exist:
    min_val_destroyed_tiles=1
    min_train_destroyed_tiles=1

IMPORTANT:
- Unsharp+Edge is TRAIN augmentation only.
- VAL/TEST use RAW RGB (fusion_p forced to 0.0 for val/test loaders)
- Calibration is fit on RAW VAL logits.
...
# =============================================================================
# Fusion: Unsharp + Edge (BGR uint8 -> BGR uint8)
# =============================================================================
...
# =============================================================================
# Dataset: DESTROYED-AWARE CROPS + Unsharp+Edge on POST (TRAIN AUG ONLY)
# =============================================================================
...
def train_and_eval_pipeline_one(args, init_weight: str):
    ...
    # IMPORTANT: VAL/TEST use raw RGB (fusion_p=0.0)
...
def main():
    ...
    # IMPORTANT: AMP ON BY DEFAULT (so you don't need --amp)
    ap.add_argument("--no_amp", action="store_true", help="Disable AMP (AMP is enabled by default).")
    ...
    # These defaults match your requested baked-in CLI flags
    ap.add_argument("--fusion_p", type=float, default=0.75,
                    help="TRAIN-only augmentation probability for Unsharp+Edge on POST.")
    ...
    # Convert new flag to the old boolean used everywhere else
    ...
    print(f"  amp          : {args.amp} (default ON; disable with --no_amp)")
    ...
    print(f"  ensure tiles : min_val_destroyed_tiles={args.min_val_destroyed_tiles}, "
          f"min_train_destroyed_tiles={args.min_train_destroyed_tiles}")
