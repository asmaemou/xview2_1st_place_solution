...
"""
================================================================================
STAGE-2 DAMAGE CLASSIFICATION (IDABD) — RGB + UNSHARP ONLY (NO OTHER FUSION)
...
RUNS WITH JUST:
  python train_idabd_stage2_pipeline_RGB_unsharp_only_earlyStop.py

What this experiment is:
- Input is still 6-channel concat: [pre_RGB, post_RGB_used]
- BUT post_RGB_used = blend(post_RGB, unsharp(post_RGB), alpha)
- No edges / no histogram equalization / no CLAHE — ONLY unsharp

Why this design:
- Keeps input at 6 channels so it remains compatible with xView2 "Double" models.
...
# =============================================================================
# Unsharp filter (BGR uint8 -> BGR uint8)
# =============================================================================
def unsharp_bgr(img_bgr: np.ndarray, amount: float = 1.0, sigma: float = 1.0) -> np.ndarray:
    """
    Classic unsharp masking:
      sharp = img*(1+amount) + blur*(-amount)
    """
    if amount <= 0:
        return img_bgr
    # sigma controls blur; ksize=(0,0) lets OpenCV infer from sigma
...
def blend_bgr(a: np.ndarray, b: np.ndarray, alpha: float) -> np.ndarray:
    ...
    if alpha >= 1:
        return b
    out = cv2.addWeighted(a, 1.0 - alpha, b, alpha, 0.0)
    return out


# =============================================================================
# Dataset (Stage-2): DESTROYED-AWARE CROPS + UNSHARP on POST (train + eval)
# =============================================================================
class IdaBDStage2Damage(Dataset):
    def __init__(
        ...
        # unsharp config
        self.unsharp_p = float(unsharp_p)
    ...
    def __getitem__(self, idx):
        ...
        # Apply unsharp to POST with probability p (train),
        # and also in eval (we want the same input definition for this experiment).
        do_unsharp = (self.rng.random() < self.unsharp_p) if self.is_train else (self.unsharp_p >= 1.0)
        if do_unsharp:
            post_u = unsharp_bgr(post, amount=self.unsharp_amount, sigma=self.unsharp_sigma)
            post = blend_bgr(post, post_u, alpha=self.unsharp_alpha)
...
def train_and_eval_pipeline_one(args, init_weight: str):
    ...
    # Ensure VAL has destroyed tiles
    ...
    # Ensure TRAIN has destroyed tiles
...
def main():
    ...
    # Unsharp config (THIS experiment)
    ap.add_argument("--unsharp_p", type=float, default=1.0,
                    help="Probability to apply unsharp to POST. Use 1.0 for 'RGB+unsharp only'.")
    ap.add_argument("--unsharp_alpha", type=float, default=0.6,
                    help="Blend strength: post_used=(1-alpha)*post + alpha*unsharp(post).")
    ap.add_argument("--unsharp_amount", type=float, default=1.0,
                    help="Unsharp amount (edge boost).")
    ap.add_argument("--unsharp_sigma", type=float, default=1.0,
                    help="Gaussian blur sigma for unsharp.")
    ...
    print(f"  unsharp_p     : {args.unsharp_p}")
    print(f"  unsharp_alpha : {args.unsharp_alpha}")
    print(f"  unsharp_amount: {args.unsharp_amount}")
    print(f"  unsharp_sigma : {args.unsharp_sigma}")
    ...
    if not args.loc_weight or not path.exists(args.loc_weight):
        raise FileNotFoundError(
            ...
            "Put it in idabd_stage1_loc_ft_checkpoints/ or pass --loc_weight explicitly."
        )
