#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
================================================================================
EVAL: Stage-1 LOC (RAW PRE RGB) + Stage-2 DAMAGE (6ch = [pre_rgb, post_used])

Stage-2 input:
  x6 = [pre_rgb(3), post_used_rgb(3)]  => 6xHxW

Default eval matches training VAL/TEST:
  - post_used = post_raw (identity; fusion disabled)

Optional:
  --eval_fusion_aug  -> post_used = fusion_augment(post) (deterministic)

Supports:
  - Stage-1 ensemble (all loc weights matching --loc_glob), per-model Platt if found
  - Stage-2 ensemble (all stage2 ckpts matching --stage2_glob OR auto fallback patterns)
  - Uncalibrated and (optional) calibrated damage prediction:
      vector-scaling npz (auto-found per model if exists)

RUN (auto-discover):
  python eval_idabd_stage2_pipeline_RGBplusFUSION_6ch.py

RUN (explicit dirs):
  python eval_idabd_stage2_pipeline_RGBplusFUSION_6ch.py \
    --stage2_dir idabd_stage2_damage_ft_checkpoints_RGBplusFUSION_6ch \
    --loc_dir idabd_stage1_loc_ft_checkpoints

================================================================================
"""

import os
from os import path
import sys
from pathlib import Path
import glob
import argparse
import random
import csv

import numpy as np
import cv2
import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.utils.data import Dataset, DataLoader


# ---------------------------------------------------------------------
# Make sure "zoo" is importable
# ---------------------------------------------------------------------
THIS_DIR = Path(__file__).resolve().parent
for cand in [THIS_DIR, THIS_DIR.parent, THIS_DIR.parent.parent]:
    if (cand / "zoo").is_dir():
        sys.path.insert(0, str(cand))
        break


# ---------------------------------------------------------------------
# Defaults
# ---------------------------------------------------------------------
IMG_DIR = "../idabd/images"
MASK_DIR = "../idabd/masks"

DEFAULT_STAGE2_DIR_CANDIDATES = [
    "idabd_stage2_damage_ft_checkpoints_RGBplusFUSION_6ch",
    "idabd_stage2_damage_ft_checkpoints_RGBplusFUSION",
    "idabd_stage2_damage_ft_checkpoints",
    "stage2_damage_checkpoints",
    "stage2_checkpoints",
]

DEFAULT_LOC_DIR_CANDIDATES = [
    "idabd_stage1_loc_ft_checkpoints",
    "stage1_loc_checkpoints",
    "loc_checkpoints",
]

IMAGENET_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32)
IMAGENET_STD  = np.array([0.229, 0.224, 0.225], dtype=np.float32)

IGNORE_LABEL = 255
CLASS_NAMES_4 = ["No-Damage", "Minor", "Major", "Destroyed"]
BUILD_TENSOR_CPU = torch.tensor([1, 2, 3, 4], dtype=torch.long)

STAGE2_IN_CH = 6


CSV_FIELDS = [
    "seed",
    "img_dir",
    "mask_dir",
    "gt_split",
    "val_ratio",
    "test_ratio",
    "split_mode",

    "loc_dir",
    "loc_models_used",
    "loc_glob",
    "loc_thresh",

    "stage2_dir",
    "stage2_models_used",
    "stage2_glob_used",
    "stage2_max_models",

    "eval_fusion_aug",
    "use_damage_calib",

    "pipeline_acc_uncal",
    "pipeline_loc_precision_uncal",
    "pipeline_loc_recall_uncal",
    "pipeline_loc_f1_uncal",
    "pipeline_macroF1_damage_uncal",
    "pipeline_f1_no_damage_uncal",
    "pipeline_f1_minor_uncal",
    "pipeline_f1_major_uncal",
    "pipeline_f1_destroyed_uncal",

    "pipeline_acc_cal",
    "pipeline_loc_precision_cal",
    "pipeline_loc_recall_cal",
    "pipeline_loc_f1_cal",
    "pipeline_macroF1_damage_cal",
    "pipeline_f1_no_damage_cal",
    "pipeline_f1_minor_cal",
    "pipeline_f1_major_cal",
    "pipeline_f1_destroyed_cal",
]


def ensure_csv(csv_path: str, overwrite: bool = False):
    if not csv_path:
        return
    out_dir = path.dirname(csv_path)
    if out_dir:
        os.makedirs(out_dir, exist_ok=True)

    if overwrite and path.exists(csv_path):
        os.remove(csv_path)

    if not path.exists(csv_path):
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(f, fieldnames=CSV_FIELDS)
            w.writeheader()
            f.flush()


def append_csv_row(csv_path: str, row: dict):
    if not csv_path:
        return
    with open(csv_path, "a", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=CSV_FIELDS, extrasaction="ignore")
        w.writerow(row)
        f.flush()


# =============================================================================
# Triplets
# =============================================================================
def tile_id_from_name(fname: str) -> str:
    base = path.splitext(path.basename(fname))[0]
    base = base.replace("_pre_disaster", "").replace("_post_disaster", "")
    return base


def list_split_files(root: str, split: str):
    exts = ("*.png", "*.jpg", "*.jpeg", "*.tif", "*.tiff", "*.bmp")
    files = []
    for e in exts:
        files += glob.glob(path.join(root, e))
    files = sorted(files)
    if split == "pre":
        files = [f for f in files if "_pre_disaster" in path.basename(f)]
    elif split == "post":
        files = [f for f in files if "_post_disaster" in path.basename(f)]
    return files


def find_mask(mask_dir: str, tile_id: str, gt_split: str):
    for ext in (".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp"):
        cand = path.join(mask_dir, f"{tile_id}_{gt_split}_disaster{ext}")
        if path.exists(cand):
            return cand
    return None


def build_stage2_triplets(img_dir: str, mask_dir: str, gt_split: str = "post"):
    pre_imgs  = list_split_files(img_dir, "pre")
    post_imgs = list_split_files(img_dir, "post")

    pre_map  = {tile_id_from_name(p): p for p in pre_imgs}
    post_map = {tile_id_from_name(p): p for p in post_imgs}

    tile_ids = sorted(set(pre_map.keys()) & set(post_map.keys()))
    triplets = []
    for tid in tile_ids:
        m = find_mask(mask_dir, tid, gt_split)
        if m is None:
            continue
        triplets.append((pre_map[tid], post_map[tid], m))
    return triplets


# =============================================================================
# Fusion augmentation (deterministic)
# =============================================================================
def canny_edges_rgb(img_bgr: np.ndarray) -> np.ndarray:
    gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
    edges = cv2.Canny(gray, 50, 150)
    return cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)


def hist_equalize_rgb(img_bgr: np.ndarray) -> np.ndarray:
    ycrcb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2YCrCb)
    ycrcb[:, :, 0] = cv2.equalizeHist(ycrcb[:, :, 0])
    return cv2.cvtColor(ycrcb, cv2.COLOR_YCrCb2BGR)


def unsharp_mask_rgb(img_bgr: np.ndarray, amount=1.0, radius=3) -> np.ndarray:
    blur = cv2.GaussianBlur(img_bgr, (0, 0), radius)
    return cv2.addWeighted(img_bgr, 1 + amount, blur, -amount, 0)


def clahe_rgb(img_bgr: np.ndarray, clip=2.0, tile=(8, 8)) -> np.ndarray:
    lab = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2LAB)
    l, a, b = cv2.split(lab)
    clahe = cv2.createCLAHE(clipLimit=clip, tileGridSize=tile)
    l2 = clahe.apply(l)
    lab2 = cv2.merge([l2, a, b])
    return cv2.cvtColor(lab2, cv2.COLOR_LAB2BGR)


def fusion_augment(img_bgr: np.ndarray) -> np.ndarray:
    e = canny_edges_rgb(img_bgr)
    h = hist_equalize_rgb(img_bgr)
    u = unsharp_mask_rgb(img_bgr, amount=1.0, radius=3)
    c = clahe_rgb(img_bgr, clip=2.0, tile=(8, 8))
    fused = (
        0.60 * img_bgr.astype(np.float32) +
        0.10 * e.astype(np.float32) +
        0.10 * h.astype(np.float32) +
        0.10 * u.astype(np.float32) +
        0.10 * c.astype(np.float32)
    )
    return np.clip(fused, 0, 255).astype(np.uint8)


# =============================================================================
# Preprocess
# =============================================================================
def pad_to_factor(img, factor=32):
    h, w = img.shape[:2]
    new_h = int(np.ceil(h / factor) * factor)
    new_w = int(np.ceil(w / factor) * factor)
    pad_h = new_h - h
    pad_w = new_w - w
    if pad_h == 0 and pad_w == 0:
        return img, (0, 0)
    padded = cv2.copyMakeBorder(img, 0, pad_h, 0, pad_w, borderType=cv2.BORDER_REFLECT_101)
    return padded, (pad_h, pad_w)


def preprocess_rgb(img_bgr: np.ndarray) -> torch.Tensor:
    img = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
    img = (img - IMAGENET_MEAN) / IMAGENET_STD
    img = np.transpose(img, (2, 0, 1))  # CHW
    return torch.from_numpy(img)


def preprocess_6ch(pre_bgr, post_bgr_used) -> torch.Tensor:
    pre  = preprocess_rgb(pre_bgr)
    post = preprocess_rgb(post_bgr_used)
    return torch.cat([pre, post], dim=0)  # 6xHxW


def load_mask_raw(mask_path: str) -> np.ndarray:
    m = cv2.imread(mask_path, cv2.IMREAD_UNCHANGED)
    if m is None:
        raise FileNotFoundError(mask_path)
    if m.ndim == 3:
        m = cv2.cvtColor(m, cv2.COLOR_BGR2GRAY)
    return m.astype(np.int64)


class IdaBDEvalRGBPlusFusion6ch(Dataset):
    """
    Returns:
      - x6 (6ch) for stage-2
      - pre_raw3 (3ch) for stage-1 loc
      - y mask
    """
    def __init__(self, triplets, eval_fusion_aug: bool = False):
        self.triplets = triplets
        self.eval_fusion_aug = bool(eval_fusion_aug)

    def __len__(self):
        return len(self.triplets)

    def __getitem__(self, idx):
        pre_path, post_path, mask_path = self.triplets[idx]

        pre  = cv2.imread(pre_path, cv2.IMREAD_COLOR)
        post = cv2.imread(post_path, cv2.IMREAD_COLOR)
        if pre is None:
            raise FileNotFoundError(pre_path)
        if post is None:
            raise FileNotFoundError(post_path)

        m = load_mask_raw(mask_path)

        pre, pad_hw = pad_to_factor(pre, 32)
        post, _ = pad_to_factor(post, 32)
        if pad_hw != (0, 0):
            m = np.pad(m, ((0, pad_hw[0]), (0, pad_hw[1])), mode="edge")

        pre_raw3 = preprocess_rgb(pre)

        if self.eval_fusion_aug:
            post_used = fusion_augment(post)
        else:
            post_used = post

        x6 = preprocess_6ch(pre, post_used)
        y = torch.from_numpy(m).long()
        return x6.float(), pre_raw3.float(), y.long(), path.basename(post_path)


# =============================================================================
# Checkpoint loading + model building
# =============================================================================
def strip_prefixes(sd: dict):
    prefixes = ["module.", "model.", "net."]
    out = {}
    for k, v in sd.items():
        kk = k
        for p in prefixes:
            if kk.startswith(p):
                kk = kk[len(p):]
        out[kk] = v
    return out


def load_state_dict_any(weight_path: str):
    try:
        ckpt = torch.load(weight_path, map_location="cpu", weights_only=False)
    except TypeError:
        ckpt = torch.load(weight_path, map_location="cpu")

    if isinstance(ckpt, dict) and "state_dict" in ckpt:
        sd = ckpt["state_dict"]
    elif isinstance(ckpt, dict) and "model" in ckpt:
        sd = ckpt["model"]
    else:
        sd = ckpt

    if not isinstance(sd, dict):
        raise ValueError(f"Checkpoint not a state_dict mapping: {weight_path}")
    return strip_prefixes(sd), ckpt


def detect_backbone_from_filename(fname_lower: str) -> str:
    for key in ["dpn92", "res34", "res50", "se154"]:
        if key in fname_lower:
            return key
    return ""


def build_damage_model_from_ckpt(stage2_ckpt_path: str) -> nn.Module:
    import zoo.models as zm
    fname = path.basename(stage2_ckpt_path).lower()
    bb = detect_backbone_from_filename(fname)
    if bb == "dpn92":
        model = zm.Dpn92_Unet_Double(pretrained=None)
    elif bb == "res34":
        model = zm.Res34_Unet_Double(pretrained=False)
    elif bb == "res50":
        model = zm.SeResNext50_Unet_Double(pretrained=None)
    elif bb == "se154":
        model = zm.SeNet154_Unet_Double(pretrained=None)
    else:
        raise ValueError(f"Unrecognized stage-2 backbone: {fname}")

    sd, _ = load_state_dict_any(stage2_ckpt_path)
    model.load_state_dict(sd, strict=True)
    return model


def build_loc_model_from_weight(weight_path: str) -> nn.Module:
    import zoo.models as zm
    fname = path.basename(weight_path).lower()
    bb = detect_backbone_from_filename(fname)
    if bb == "dpn92":
        model = zm.Dpn92_Unet_Loc(pretrained=None)
    elif bb == "res34":
        model = zm.Res34_Unet_Loc(pretrained=False)
    elif bb == "res50":
        model = zm.SeResNext50_Unet_Loc(pretrained=None)
    elif bb == "se154":
        model = zm.SeNet154_Unet_Loc(pretrained=None)
    else:
        raise ValueError(f"Unrecognized stage-1 backbone: {fname}")

    sd, _ = load_state_dict_any(weight_path)
    model.load_state_dict(sd, strict=True)
    return model


# =============================================================================
# Metrics + ensembles
# =============================================================================
def _confusion_add(conf, gt_flat, pr_flat, ncls):
    idx = (gt_flat * ncls + pr_flat).astype(np.int64)
    binc = np.bincount(idx, minlength=ncls * ncls)
    conf += binc.reshape(ncls, ncls)


def f1s_from_conf(conf):
    n = conf.shape[0]
    f1s = []
    for c in range(n):
        tp = conf[c, c]
        fp = conf[:, c].sum() - tp
        fn = conf[c, :].sum() - tp
        f1 = (2 * tp) / (2 * tp + fp + fn + 1e-9)
        f1s.append(float(f1))
    return f1s


def acc_from_conf(conf):
    return float(np.trace(conf) / (conf.sum() + 1e-9))


def prf_from_counts(tp, fp, fn):
    p = tp / (tp + fp + 1e-9)
    r = tp / (tp + fn + 1e-9)
    f = (2 * tp) / (2 * tp + fp + fn + 1e-9)
    return float(p), float(r), float(f)


def make_amp_ctx(device: torch.device, use_amp: bool):
    if use_amp and hasattr(torch, "amp") and hasattr(torch.amp, "autocast"):
        def _ctx():
            return torch.amp.autocast(device_type=device.type, enabled=(device.type == "cuda"))
        return _ctx
    else:
        def _ctx():
            return torch.cuda.amp.autocast(enabled=(use_amp and device.type == "cuda"))
        return _ctx


@torch.no_grad()
def predict_build_mask_ensemble(loc_pack, pre_raw3, thresh, amp_ctx):
    prob_sum = None
    for (m, a, b) in loc_pack:
        with amp_ctx():
            logit = m(pre_raw3)
            if isinstance(logit, (tuple, list)):
                logit = logit[0]
            if logit.ndim == 3:
                logit = logit.unsqueeze(1)
            if logit.ndim == 4 and logit.shape[1] > 1:
                logit = logit[:, 0:1, :, :]
            logit = a * logit + b
            prob = torch.sigmoid(logit)[:, 0, :, :]
        prob_sum = prob if prob_sum is None else (prob_sum + prob)

    prob_mean = prob_sum / float(len(loc_pack))
    return prob_mean >= thresh


def apply_vector_scaling(z_b4hw: torch.Tensor, W: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    B, C, H, Wd = z_b4hw.shape
    z = z_b4hw.permute(0, 2, 3, 1).contiguous().view(-1, 4)
    z = z @ W.t() + b
    z = z.view(B, H, Wd, 4).permute(0, 3, 1, 2).contiguous()
    return z


@torch.no_grad()
def predict_damage_ensemble(stage2_pack, x6, amp_ctx, use_calib: bool):
    prob_sum = None
    for item in stage2_pack:
        m = item["model"]
        W = item.get("calib_W", None)
        b = item.get("calib_b", None)

        with amp_ctx():
            logits = m(x6)
            if isinstance(logits, (tuple, list)):
                logits = logits[0]
            z = logits[:, 1:5, :, :]

        if use_calib and (W is not None) and (b is not None):
            z = z.float()
            z = apply_vector_scaling(z, W, b)
            probs = F.softmax(z, dim=1)
        else:
            probs = F.softmax(z, dim=1)

        prob_sum = probs if prob_sum is None else (prob_sum + probs)

    prob_mean = prob_sum / float(len(stage2_pack))
    return torch.argmax(prob_mean, dim=1) + 1


@torch.no_grad()
def eval_pipeline(loader, device, loc_pack, stage2_pack, loc_thresh, use_amp: bool, use_calib: bool):
    amp_ctx = make_amp_ctx(device, use_amp)

    conf5 = np.zeros((5, 5), dtype=np.int64)
    loc_tp = 0.0
    loc_fp = 0.0
    loc_fn = 0.0
    build_tensor = BUILD_TENSOR_CPU.to(device)

    for x6, pre_raw3, raw, _ in loader:
        x6 = x6.to(device, non_blocking=True)
        pre_raw3 = pre_raw3.to(device, non_blocking=True)
        raw = raw.to(device, non_blocking=True)

        build_mask = predict_build_mask_ensemble(loc_pack, pre_raw3, loc_thresh, amp_ctx=amp_ctx)
        pred_damage = predict_damage_ensemble(stage2_pack, x6, amp_ctx=amp_ctx, use_calib=use_calib)

        pred_final = torch.zeros_like(pred_damage)
        pred_final[build_mask] = pred_damage[build_mask]

        valid = (raw != IGNORE_LABEL)
        gt_build = valid & torch.isin(raw, build_tensor)
        pr_build = valid & build_mask

        loc_tp += float((pr_build & gt_build).sum().item())
        loc_fp += float((pr_build & (~gt_build)).sum().item())
        loc_fn += float(((~pr_build) & gt_build).sum().item())

        gt_np = raw.detach().cpu().numpy().astype(np.int64)
        pr_np = pred_final.detach().cpu().numpy().astype(np.int64)

        for i in range(gt_np.shape[0]):
            g = gt_np[i]
            p = pr_np[i]
            v = (g != IGNORE_LABEL)
            if v.sum() == 0:
                continue
            _confusion_add(conf5, g[v], np.clip(p[v], 0, 4), 5)

    acc5 = acc_from_conf(conf5)
    f1s5 = f1s_from_conf(conf5)
    f1s_damage = [f1s5[c] for c in [1, 2, 3, 4]]
    macro_damage = float(np.mean(f1s_damage))
    loc_p, loc_r, loc_f1 = prf_from_counts(loc_tp, loc_fp, loc_fn)
    return acc5, loc_p, loc_r, loc_f1, macro_damage, f1s_damage


# =============================================================================
# Loading packs (LOC + STAGE2)
# =============================================================================
def strip_known_suffix(name_noext: str, suffixes):
    for suf in suffixes:
        if name_noext.endswith(suf):
            return name_noext[:-len(suf)]
    return name_noext


def load_loc_pack(loc_dir: str, device, loc_glob="*_idabd_ft_best.pth"):
    weights = sorted(glob.glob(path.join(loc_dir, loc_glob)))
    if not weights:
        # fallback: take best/last
        weights = sorted(glob.glob(path.join(loc_dir, "*loc*best*.pth"))) + sorted(glob.glob(path.join(loc_dir, "*loc*last*.pth")))
        weights = [w for w in weights if path.isfile(w)]
        weights = sorted(list(dict.fromkeys(weights)))

    if not weights:
        raise FileNotFoundError(f"No Stage-1 loc weights found in {loc_dir}")

    pack = []
    for w in weights:
        base = path.basename(w)
        noext = path.splitext(base)[0]
        stem = strip_known_suffix(noext, ["_idabd_ft_best", "_idabd_ft_last"])
        platt = path.join(loc_dir, f"{stem}_idabd_platt.npz")

        model = build_loc_model_from_weight(w).to(device).eval()
        for p in model.parameters():
            p.requires_grad = False

        a, b = 1.0, 0.0
        if path.exists(platt):
            d = np.load(platt)
            if ("a" in d.files) and ("b" in d.files):
                a = float(d["a"])
                b = float(d["b"])
            else:
                print(f"[WARN] Platt npz missing a/b keys: {platt} -> using a=1,b=0")

        pack.append((model, a, b))

    return pack, weights


def find_calib_for_stage2_ckpt(stage2_ckpt_path: str) -> str:
    """
    Try hard to locate a calibration npz near the checkpoint.

    Accepts:
      *_idabd_stage2_calib_vector_scaling_6ch.npz
      *_calib_vector_scaling_6ch.npz
      *_vector_scaling_6ch.npz
      *_vector_scaling*.npz (fallback)
    """
    d = path.dirname(stage2_ckpt_path)
    b = path.basename(stage2_ckpt_path)
    noext = path.splitext(b)[0]

    # most likely: base extracted from known suffix
    suffixes = ["_idabd_stage2_ft_best_6ch", "_idabd_stage2_ft_best", "_idabd_stage2_ft_last_6ch", "_idabd_stage2_ft_last"]
    base = noext
    for s in suffixes:
        if noext.endswith(s):
            base = noext[:-len(s)]
            break

    candidates = [
        path.join(d, f"{base}_idabd_stage2_calib_vector_scaling_6ch.npz"),
        path.join(d, f"{base}_calib_vector_scaling_6ch.npz"),
        path.join(d, f"{base}_vector_scaling_6ch.npz"),
    ]
    for c in candidates:
        if path.exists(c):
            return c

    # fallback: any calib npz in same dir
    globs = [
        path.join(d, f"{base}*_vector_scaling_6ch.npz"),
        path.join(d, "*_idabd_stage2_calib_vector_scaling_6ch.npz"),
        path.join(d, "*_vector_scaling_6ch.npz"),
        path.join(d, "*vector_scaling*.npz"),
    ]
    hits = []
    for g in globs:
        hits += glob.glob(g)
    hits = [h for h in hits if path.isfile(h)]
    if not hits:
        return ""
    # pick most recent
    return max(hits, key=lambda p: os.path.getmtime(p))


def load_stage2_pack(stage2_dir: str, device, stage2_glob: str, stage2_max_models: int, use_damage_calib: bool):
    """
    Robust stage2 discovery:
      1) try user glob
      2) try common globs
      3) fallback: any *.pth in stage2_dir
    """
    tried = []

    def _try(glb):
        tried.append(glb)
        return sorted(glob.glob(path.join(stage2_dir, glb)))

    ckpts = _try(stage2_glob)

    if not ckpts:
        for glb in [
            "*_idabd_stage2_ft_best_6ch.pth",
            "*_idabd_stage2_ft_best.pth",
            "*_ft_best*.pth",
            "*best*.pth",
        ]:
            ckpts = _try(glb)
            if ckpts:
                stage2_glob = glb
                break

    if not ckpts:
        ckpts = sorted(glob.glob(path.join(stage2_dir, "*.pth")))
        ckpts = [c for c in ckpts if path.isfile(c)]
        stage2_glob = "*.pth"

    ckpts = [c for c in ckpts if path.isfile(c)]
    if not ckpts:
        raise FileNotFoundError(f"No Stage-2 checkpoints found in {stage2_dir}. Tried: {tried}")

    # sort by mtime descending (most recent first)
    ckpts = sorted(ckpts, key=lambda p: os.path.getmtime(p), reverse=True)

    if stage2_max_models > 0 and len(ckpts) > stage2_max_models:
        ckpts = ckpts[:stage2_max_models]

    pack = []
    used_calib = 0
    for ck in ckpts:
        m = build_damage_model_from_ckpt(ck).to(device).eval()
        for p in m.parameters():
            p.requires_grad = False

        item = {"model": m, "ckpt": ck, "calib_path": "", "calib_W": None, "calib_b": None}

        if use_damage_calib:
            calib_path = find_calib_for_stage2_ckpt(ck)
            if calib_path and path.exists(calib_path):
                d = np.load(calib_path)
                if ("W" in d.files) and ("b" in d.files):
                    item["calib_path"] = calib_path
                    item["calib_W"] = torch.from_numpy(d["W"]).to(device=device, dtype=torch.float32)
                    item["calib_b"] = torch.from_numpy(d["b"]).to(device=device, dtype=torch.float32)
                    used_calib += 1
                else:
                    print(f"[WARN] calib npz missing W/b keys: {calib_path} -> ignoring")

        pack.append(item)

    return pack, ckpts, used_calib, stage2_glob


# =============================================================================
# Auto dirs
# =============================================================================
def _script_dir() -> Path:
    return Path(__file__).resolve().parent


def _candidate_dirs(*rel_paths):
    roots = [_script_dir(), _script_dir().parent, _script_dir().parent.parent, Path.cwd(), Path.cwd().parent]
    out = []
    for r in roots:
        for rel in rel_paths:
            d = (r / rel).resolve()
            if d.is_dir():
                out.append(str(d))
    seen, dedup = set(), []
    for d in out:
        if d not in seen:
            dedup.append(d)
            seen.add(d)
    return dedup


def auto_find_dataset_dirs():
    img_dirs = _candidate_dirs("idabd/images", "../idabd/images", "data/idabd/images", "dataset/idabd/images")
    mask_dirs = _candidate_dirs("idabd/masks", "../idabd/masks", "data/idabd/masks", "dataset/idabd/masks")
    return (img_dirs[0] if img_dirs else IMG_DIR), (mask_dirs[0] if mask_dirs else MASK_DIR)


def auto_find_dir(candidates):
    rels = []
    for c in candidates:
        rels += [c, f"../{c}", f"two_stage_pipeline/{c}", f"../two_stage_pipeline/{c}"]
    dirs = _candidate_dirs(*rels)
    return (dirs[0] if dirs else "")


# =============================================================================
# Main
# =============================================================================
def main():
    ap = argparse.ArgumentParser()

    ap.add_argument("--img_dir", default="")
    ap.add_argument("--mask_dir", default="")
    ap.add_argument("--gt_split", choices=["pre", "post"], default="post")

    ap.add_argument("--val_ratio", type=float, default=0.1)
    ap.add_argument("--test_ratio", type=float, default=0.1)
    ap.add_argument("--seed", type=int, default=0)

    ap.add_argument("--split_mode", choices=["test_first", "test_last"], default="test_first")

    ap.add_argument("--loc_dir", default="")
    ap.add_argument("--loc_glob", default="*_idabd_ft_best.pth")
    ap.add_argument("--loc_thresh", type=float, default=0.5)

    ap.add_argument("--stage2_dir", default="")
    ap.add_argument("--stage2_glob", default="*_idabd_stage2_ft_best_6ch.pth")
    ap.add_argument("--stage2_max_models", type=int, default=12)

    ap.add_argument("--eval_fusion_aug", action="store_true")
    ap.add_argument("--use_damage_calib", action="store_true")

    ap.add_argument("--amp", action="store_true")
    ap.add_argument("--workers", type=int, default=2)

    ap.add_argument("--csv_path", default="idabd_eval_pipeline_RGBplusFUSION_6ch.csv")
    ap.add_argument("--overwrite_csv", action="store_true")

    args = ap.parse_args()
    if args.val_ratio + args.test_ratio >= 1.0:
        raise ValueError("val_ratio + test_ratio must be < 1.0")

    img_auto, mask_auto = auto_find_dataset_dirs()
    if not args.img_dir or not path.isdir(args.img_dir):
        args.img_dir = img_auto
    if not args.mask_dir or not path.isdir(args.mask_dir):
        args.mask_dir = mask_auto

    if not args.loc_dir:
        args.loc_dir = auto_find_dir(DEFAULT_LOC_DIR_CANDIDATES)
    if not args.stage2_dir:
        args.stage2_dir = auto_find_dir(DEFAULT_STAGE2_DIR_CANDIDATES)

    print("\n[AUTO CONFIG]")
    print(f"  img_dir    : {args.img_dir}")
    print(f"  mask_dir   : {args.mask_dir}")
    print(f"  gt_split   : {args.gt_split}")
    print(f"  loc_dir    : {args.loc_dir if args.loc_dir else '(NOT FOUND)'}")
    print(f"  stage2_dir : {args.stage2_dir if args.stage2_dir else '(NOT FOUND)'}")

    if not path.isdir(args.img_dir):
        raise FileNotFoundError("Could not find img_dir. Pass --img_dir explicitly.")
    if not path.isdir(args.mask_dir):
        raise FileNotFoundError("Could not find mask_dir. Pass --mask_dir explicitly.")
    if not args.loc_dir or not path.isdir(args.loc_dir):
        raise FileNotFoundError("Could not find loc_dir. Pass --loc_dir explicitly.")
    if not args.stage2_dir or not path.isdir(args.stage2_dir):
        raise FileNotFoundError("Could not find stage2_dir. Pass --stage2_dir explicitly.")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"[DEVICE] {device}")

    random.seed(args.seed)
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(args.seed)

    ensure_csv(args.csv_path, overwrite=args.overwrite_csv)
    print(f"[CSV   ] Ready: {args.csv_path}")

    triplets = build_stage2_triplets(args.img_dir, args.mask_dir, gt_split=args.gt_split)
    if not triplets:
        raise FileNotFoundError("No (pre, post, mask) triplets found.")

    rng = random.Random(args.seed)
    rng.shuffle(triplets)
    n = len(triplets)

    n_test = max(1, int(n * args.test_ratio))
    n_val  = max(1, int(n * args.val_ratio))
    if n_test + n_val >= n:
        n_test = max(1, min(n_test, n - 2))
        n_val  = max(1, min(n_val,  n - 1 - n_test))

    if args.split_mode == "test_first":
        test_tr  = triplets[:n_test]
        val_tr   = triplets[n_test:n_test+n_val]
        train_tr = triplets[n_test+n_val:]
    else:
        train_tr = triplets[: n - (n_val + n_test)]
        val_tr   = triplets[n - (n_val + n_test) : n - n_test]
        test_tr  = triplets[n - n_test :]

    print(f"[DATA  ] total={n} | train={len(train_tr)} | val={len(val_tr)} | test={len(test_tr)}")

    test_ds = IdaBDEvalRGBPlusFusion6ch(test_tr, eval_fusion_aug=args.eval_fusion_aug)
    test_ld = DataLoader(
        test_ds,
        batch_size=1,
        shuffle=False,
        num_workers=max(0, args.workers),
        pin_memory=True
    )

    loc_pack, loc_used = load_loc_pack(args.loc_dir, device=device, loc_glob=args.loc_glob)
    print(f"[LOC   ] models: {len(loc_used)}")
    for w in loc_used:
        print("   -", w)

    stage2_pack, stage2_used, calib_loaded, stage2_glob_used = load_stage2_pack(
        args.stage2_dir,
        device=device,
        stage2_glob=args.stage2_glob,
        stage2_max_models=args.stage2_max_models,
        use_damage_calib=args.use_damage_calib,
    )
    print(f"[STAGE2] models: {len(stage2_used)} (glob_used={stage2_glob_used})")
    for item in stage2_pack:
        if args.use_damage_calib and item["calib_path"]:
            print(f"   - {item['ckpt']}  | calib: {item['calib_path']}")
        else:
            print(f"   - {item['ckpt']}")
    if args.use_damage_calib:
        print(f"[CALIB] loaded for {calib_loaded}/{len(stage2_used)} stage-2 models")

    acc_u, locP_u, locR_u, locF_u, macroD_u, f1sD_u = eval_pipeline(
        test_ld, device,
        loc_pack=loc_pack,
        stage2_pack=stage2_pack,
        loc_thresh=args.loc_thresh,
        use_amp=args.amp,
        use_calib=False,
    )

    print("\n==================== PIPELINE TEST (UNCAL) ====================")
    print("Stage-1 loc gating (raw pre RGB) -> Stage-2 damage (x6=[pre,post_used])")
    print(f"pipeline_acc(0..4)={acc_u:.6f}")
    print(f"F1 Localization (Building vs Background)={locF_u:.6f} (P={locP_u:.6f}, R={locR_u:.6f})")
    print(f"macroF1(Damage 1..4)={macroD_u:.6f}")
    for i in range(4):
        print(f"F1 {CLASS_NAMES_4[i]:>9s}: {f1sD_u[i]:.6f}")
    print("==============================================================\n")

    if args.use_damage_calib:
        acc_c, locP_c, locR_c, locF_c, macroD_c, f1sD_c = eval_pipeline(
            test_ld, device,
            loc_pack=loc_pack,
            stage2_pack=stage2_pack,
            loc_thresh=args.loc_thresh,
            use_amp=args.amp,
            use_calib=True,
        )

        print("\n==================== PIPELINE TEST (CAL) ======================")
        print("Stage-1 loc gating -> Stage-2 damage + vector scaling")
        print(f"pipeline_acc(0..4)={acc_c:.6f}")
        print(f"F1 Localization (Building vs Background)={locF_c:.6f} (P={locP_c:.6f}, R={locR_c:.6f})")
        print(f"macroF1(Damage 1..4)={macroD_c:.6f}")
        for i in range(4):
            print(f"F1 {CLASS_NAMES_4[i]:>9s}: {f1sD_c[i]:.6f}")
        print("==============================================================\n")
    else:
        acc_c = locP_c = locR_c = locF_c = macroD_c = None
        f1sD_c = [None, None, None, None]

    row = {
        "seed": args.seed,
        "img_dir": args.img_dir,
        "mask_dir": args.mask_dir,
        "gt_split": args.gt_split,
        "val_ratio": args.val_ratio,
        "test_ratio": args.test_ratio,
        "split_mode": args.split_mode,

        "loc_dir": args.loc_dir,
        "loc_models_used": len(loc_used),
        "loc_glob": args.loc_glob,
        "loc_thresh": args.loc_thresh,

        "stage2_dir": args.stage2_dir,
        "stage2_models_used": len(stage2_used),
        "stage2_glob_used": stage2_glob_used,
        "stage2_max_models": args.stage2_max_models,

        "eval_fusion_aug": bool(args.eval_fusion_aug),
        "use_damage_calib": bool(args.use_damage_calib),

        "pipeline_acc_uncal": acc_u,
        "pipeline_loc_precision_uncal": locP_u,
        "pipeline_loc_recall_uncal": locR_u,
        "pipeline_loc_f1_uncal": locF_u,
        "pipeline_macroF1_damage_uncal": macroD_u,
        "pipeline_f1_no_damage_uncal": f1sD_u[0],
        "pipeline_f1_minor_uncal": f1sD_u[1],
        "pipeline_f1_major_uncal": f1sD_u[2],
        "pipeline_f1_destroyed_uncal": f1sD_u[3],

        "pipeline_acc_cal": acc_c,
        "pipeline_loc_precision_cal": locP_c,
        "pipeline_loc_recall_cal": locR_c,
        "pipeline_loc_f1_cal": locF_c,
        "pipeline_macroF1_damage_cal": macroD_c,
        "pipeline_f1_no_damage_cal": f1sD_c[0],
        "pipeline_f1_minor_cal": f1sD_c[1],
        "pipeline_f1_major_cal": f1sD_c[2],
        "pipeline_f1_destroyed_cal": f1sD_c[3],
    }
    append_csv_row(args.csv_path, row)
    print(f"[CSV   ] Appended -> {args.csv_path}")


if __name__ == "__main__":
    main()
