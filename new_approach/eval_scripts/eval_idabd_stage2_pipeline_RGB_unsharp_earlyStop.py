...
IDABD end-to-end pipeline — RGB + UNSHARP ONLY

RUNS WITH JUST:
  python eval_idabd_stage2_pipeline_RGB_unsharp_only_earlyStop.py

Input definition:
- x6 = [pre_RGB, post_RGB_used]
- post_RGB_used = blend(post_RGB, unsharp(post_RGB), alpha)
...
DEFAULT_STAGE2_DIR = "idabd_stage2_damage_ft_checkpoints_RGB_UNSHARP_ONLY"
...
class IdaBDEval(Dataset):
    def __init__(self, triplets, unsharp_alpha=0.6, unsharp_amount=1.0, unsharp_sigma=1.0):
    ...
    def __getitem__(self, idx):
        ...
        # Apply unsharp definition deterministically
...
def main():
    ...
    # Unsharp params MUST match training experiment definition
    ...
    ap.add_argument("--csv_path", default="idabd_pipeline_ENSEMBLE_LOC+DMG_CAL_ONLY_RGB_UNSHARP_ONLY.csv")
    ...
    test_ld = DataLoader(test_ds, batch_size=1, shuffle=False, num_workers=max(0, args.workers), pin_memory=True)
