...
IDABD end-to-end pipeline — for outputs of:
    train_idabd_stage2_pipeline_RGB_plus_EDGES_only_earlyStop.py

RUNS WITH JUST:
  python eval_idabd_stage2_pipeline_RGB_plus_EDGES_only_earlyStop.py

This evaluator:
- Stage-1 ensemble: average building probabilities (with optional Platt scaling)
- Stage-2 ensemble: average calibrated (vector scaling) softmax probabilities (classes 1..4)
- Reports calibrated-only metrics:
  F1 Localization + F1 per damage class + macroF1 damage + pipeline accuracy
================================================================================
"""
...
DEFAULT_STAGE2_DIR = "idabd_stage2_damage_ft_checkpoints_RGB+EDGES_ONLY"
...
def main():
    ...
    ap.add_argument("--csv_path", default="idabd_pipeline_ENSEMBLE_LOC+DMG_CAL_ONLY_RGB+EDGES_ONLY.csv")
    ...
    if not path.isdir(args.stage2_dir):
        if s2_tried:
            ...
            for d in s2_tried:
                print("  -", d)
        raise FileNotFoundError(f"Could not find stage2_dir: {args.stage2_dir}")
