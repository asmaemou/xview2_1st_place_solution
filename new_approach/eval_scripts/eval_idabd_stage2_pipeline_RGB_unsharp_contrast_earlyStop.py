...
IDABD end-to-end pipeline — RGB + UNSHARP + CONTRAST ONLY

RUNS WITH JUST:
  python eval_idabd_stage2_pipeline_RGB_unsharp_contrast_only_earlyStop.py

Fusion definition (must match training):
- post_used = blend(post, unsharp(post), alpha_unsharp)
- then contrast CLAHE on LAB-L and blend with alpha_contrast
...
DEFAULT_STAGE2_DIR = "idabd_stage2_damage_ft_checkpoints_RGB_UNSHARP_CONTRAST_ONLY"
...
def fuse_unsharp_contrast_post(
    ...
    return blend_bgr(post_m, post_c, alpha_contrast)
...
class IdaBDEval(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        post = fuse_unsharp_contrast_post(
            post,
...
# =============================================================================
# Model loading + ensembling + eval (same as your ensemble eval style)
# =============================================================================
...
def main():
    ...
    ap.add_argument("--csv_path", default="idabd_pipeline_ENSEMBLE_LOC+DMG_CAL_ONLY_RGB_UNSHARP_CONTRAST_ONLY.csv")
