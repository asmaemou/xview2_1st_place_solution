...
✅ Compatible with outputs from:
   train_idabd_stage2_pipeline_Fusion_earlyStop.py
...
Stage-2 ensemble (damage):
  - Load multiple stage-2 fine-tuned damage models (best checkpoints)
...
def detect_backbone_from_filename(fname_lower: str) -> str:
    """
    Robust backbone detection even if filename doesn't start with the backbone.
    """
...
def load_loc_pack(loc_dir: str, device, loc_glob="*_idabd_ft_best.pth"):
    ...
    for w in weights:
        ...
        # training saves: <stem>_idabd_ft_best.pth and <stem>_idabd_platt.npz
...
def find_stage2_calib(stage2_dir: str, ckpt_path: str) -> str:
    ...
    # fallback: try using full noext as stem (in case naming differs)
    cand2 = path.join(stage2_dir, f"{noext}_idabd_stage2_calib_vector_scaling.npz")
    if path.exists(cand2):
        return cand2

    # last resort: try glob close matches
    ...
    for ck in ckpts:
        ...
        if not calib:
            raise FileNotFoundError(
                ...
                f"Expected something like:\n  <stem>_idabd_stage2_calib_vector_scaling.npz\n"
...
def main():
    ...
    # split (must match the seed/ratios you used in training)
    ...
    ap.add_argument("--stage2_glob", default="*_idabd_stage2_ft_best.pth", help="Pattern to select Stage-2 weights in stage2_dir")
