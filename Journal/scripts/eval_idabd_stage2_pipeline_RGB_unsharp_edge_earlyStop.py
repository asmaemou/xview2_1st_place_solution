...
IDABD end-to-end pipeline — RGB + UNSHARP + EDGE ONLY

RUNS WITH JUST:
  python eval_idabd_stage2_pipeline_RGB_unsharp_edge_only_earlyStop.py

Key updates vs your previous eval:
  1) Defaults MATCH your training run:
       alpha_unsharp=0.45, alpha_edge=0.10, edge_dilate=0, canny_t1=80, canny_t2=200
  2) AMP is ON by default (use --no_amp to disable)
  3) Same split logic (seed/val_ratio/test_ratio) as training

Fusion definition:
- post_used = blend(post, unsharp(post), alpha_unsharp), then overlay edges(alpha_edge)

Ensembling:
- Stage-1: average building probabilities across loc models (with per-model platt a,b if found)
...
DEFAULT_STAGE2_DIR = "idabd_stage2_damage_ft_checkpoints_RGB_UNSHARP_EDGE_ONLY"
...
# =============================================================================
# Fusion: Unsharp + Edge (MATCH TRAIN)
# =============================================================================
...
class IdaBDEval(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # EVAL uses fused post (as defined by this eval script)
...
def main():
    ...
    # MUST match training fusion defaults (baked-in)
    ...
    # AMP ON by default (like your training request)
    ap.add_argument("--no_amp", action="store_true", help="Disable AMP (AMP is enabled by default).")

    ap.add_argument("--csv_path", default="idabd_pipeline_ENSEMBLE_LOC+DMG_CAL_ONLY_RGB_UNSHARP_EDGE_ONLY.csv")
